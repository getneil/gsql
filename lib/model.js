...
function defineRelationships(attributes){
  ...
  Object.keys(attributes).forEach((k)=>{
    ...
    if(attr.object){
      if(!associations.belongsTo) associations.belongsTo = {};

      associations.belongsTo[attr.object] = {
        as: k
      }
    }
    if(attr.list){
      if(!associations.hasMany) associations.hasMany = {};

      associations.hasMany[attr.list] = {}

      if(attr.via) associations.hasMany[attr.list].via = attr.via;
      if(attr.through) associations.hasMany[attr.list].through = attr.through;
    }
  });
  return associations;
}
...
class Model {
  constructor(name,modelConfiguration,sequelizeConnection){
    ...
    this.association = defineRelationships(modelConfiguration.attributes);
